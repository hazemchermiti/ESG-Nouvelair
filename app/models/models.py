...
from sqlalchemy import CheckConstraint, Date, Double, ForeignKeyConstraint, Index, Integer, String, TIMESTAMP, Text, text
...
class AviationLicense(Base):
    ...
    __table_args__ = (
        CheckConstraint("(`status` in (_utf8mb4'ACTIVE',_utf8mb4'EXPIRED',_utf8mb4'SUSPENDED',_utf8mb4'PENDING_RENEWAL'))", name='aviation_license_chk_1'),
...
class Employee(Base):
    ...
    __table_args__ = (
        CheckConstraint("(`gender` in (_utf8mb4'M',_utf8mb4'F'))", name='employee_chk_1'),
...
class Pillar(Base):
    ...
    __table_args__ = (
        CheckConstraint("(`code` in (_utf8mb4'E',_utf8mb4'S',_utf8mb4'G'))", name='pillar_chk_1'),
...
class TaxObligation(Base):
    ...
    __table_args__ = (
        CheckConstraint("(`status` in (_utf8mb4'PENDING',_utf8mb4'PAID_ON_TIME',_utf8mb4'PAID_LATE',_utf8mb4'OVERDUE'))", name='tax_obligation_chk_1'),
        Index('id', 'id', unique=True)
    )
...
class Kpi(Base):
    __tablename__ = 'kpi'
    __table_args__ = (
        CheckConstraint("(`better_direction` in (_utf8mb4'LOWER',_utf8mb4'HIGHER'))", name='kpi_chk_1'),
    ...
    pillar_id: Mapped[int] = mapped_column(BIGINT(unsigned=True), nullable=False)
...
class Training(Base):
    ...
    id: Mapped[int] = mapped_column(BIGINT(unsigned=True), primary_key=True)
    employee_id: Mapped[int] = mapped_column(BIGINT(unsigned=True), nullable=False)
...
class WorkAccident(Base):
    ...
    __table_args__ = (
        CheckConstraint("(`severity` in (_utf8mb4'MINOR',_utf8mb4'MAJOR',_utf8mb4'FATAL'))", name='work_accident_chk_1'),
...
class Anomaly(Base):
    ...
    id: Mapped[int] = mapped_column(BIGINT(unsigned=True), primary_key=True)
    kpi_id: Mapped[int] = mapped_column(BIGINT(unsigned=True), nullable=False)
...
class Recommendation(Base):
    ...
    id: Mapped[int] = mapped_column(BIGINT(unsigned=True), primary_key=True)
    anomaly_id: Mapped[int] = mapped_column(BIGINT(unsigned=True), nullable=False)
